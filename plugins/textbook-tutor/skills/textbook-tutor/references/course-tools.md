...
6. 纯偏好/提示轮无可安全补充知识时，读后传 `sections: []` 确认本轮已核对，不写待答题解法。不要用空数组跳过本应整理的已讲知识。备课批次只更新 Progress 中的教师备课与覆盖记录，不写 Notes。可使用文件编辑能力针对 Progress 对应章节合并，保留结构化状态并检查文件冲突。只有形成学生可复用的知识资料时才 update_knowledge；正文和 section_path 不含备课、进度或教学安排。
...
教学回复中的配图按 teaching-resources.md 的“课堂图片的固定展示格式”呈现：正常图片预览，下方来源行链接同一张已存在的本地图片。仅有 MCP 图片返回时不编造文件链接；先按允许的文件能力保存至课程 Picture，无法保存则保留原生显示并说明。不要把预览图嵌套在超链接中，也不自动打开侧栏。
